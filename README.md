...
1. operations manager enters client, contact, Client Account Executive, MSA, SOW, and contract files
...
Due invoices and candidate invoice upload reminders are generated by the backend system endpoint, not from the Operations Manager screen. The Render Blueprint includes a daily cron job that calls:

```bash
python backend/scripts/generate_due_invoices.py
```

Set `SENDGRID_API_KEY`, `SENDGRID_FROM_EMAIL`, `SENDGRID_FROM_NAME`, and `INVOICE_CRON_TOKEN` in Render for production email delivery and scheduled invoice generation.
